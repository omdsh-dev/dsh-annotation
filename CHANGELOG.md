...
### 新增
- **俄语文案（ru）**：在 zh / en 之外补齐第三套 —— 工具条、编辑窗口、提示、胶囊、回复引用与批注协议块全部俄语。`setLang` 接受 `ru`（`ru-RU` 由 DSH 的 locale 统一成 `ru`），zh / en 输出逐字未变。协议块里的回复格式保留拉丁写法 «Annotation N: …»：回复芯片的替换按它工作，与界面语言无关。数量词按俄语变格（1 аннотация / 2 аннотации / 5 аннотаций），由新增的 `plural()` 从词典的三种形式里选；zh / en 的单词条不受影响。

### 兼容性
- **三种语言的批注块互相可解析**：`BLOCK_HEADS`（首行哨兵）、`BLOCK_MARKERS`（隐藏手术的「提问：/Ask:/Вопрос:」）与 `PARSE_MARKERS`（反解析的段级分隔）都补上 ru，反解析的注记前缀也认识 `Примечание:`。纯批注块的快路径以前写死 `['zh','en']`，现在取词典自身的语言列表 —— 否则俄语纯批注块会走标记路径，被原文里的「提问：」切断。
- **第三种界面语言不再把批注界面退回中文**：本插件只有 zh / en 两套文案，而 DSH 的 locale 服务不止这两种。节点选定 `ru`（或任何 zh/en 之外的语言）时，`setLang` 以前把一切非 zh/en 都当成 zh，于是整套批注 UI —— 工具条、编辑窗口、提示、批注协议块 —— 全变成中文。现在 `zh`/`en`/`ru` 原样采用，其余一律英文；`test/locale-fallback.test.mjs` 固定这条规则（fr / de / zh-Hans 走英文）。
